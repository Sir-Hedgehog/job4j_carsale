...
public class CarDatabase implements TemplateBase {
    ...
    @Override
    public List<Car> getData(String firm, String photo, String time) {
        String substring = this.getResultQuery(firm, photo, time);
        return this.template(session -> session.createNativeQuery("select * from Car" + substring, Car.class).list());
    }
